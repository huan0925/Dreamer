            ...
            <?php
                if($u_id == $user_check){
                    echo"
                        <center>
                            <form method='post' enctype='multipart/form-data'>
                                <img src='cover/$user_cover' class='img-fluid' style='border-radius: 5%; width:600px;'>
                                <br><br>
                                <input id='u_cover' class='form-control' type='file' name='u_cover' value='$user_cover' style='width:500px; background: orange;'/>	
                                <br>
                    ...
                    if(isset($_POST['update'])){
                        ...
                        move_uploaded_file($cover_tmp,"cover/$u_cover");
                        $update = "update user set user_cover = '$u_cover' where user_id='$u_id'";
                        $run = mysqli_query($link,$update); 
                        if($run){
                            echo "<script>alert('您的背景已更新！')</script>";
