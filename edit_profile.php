...
<style>
	*{
	...
	#username,#password,#name,#birthday,#account,#describe,#gender,#status,#address,#u_image{
		width: 251px;
		height: 32px;
					...
					<form action="" method="post">
						<div class="input-group">
							<table>
								...
								<tr>
									<td colspan="6">
									<input class="btn btn-info" style="width: 80px; position: absolute; left:100px" type="submit" name="update" value="更改"/>
									</td>
									<td colspan="6">
									<button type="button" class="btn btn-info" style="width: 80px;position: relative; left:-100px; top: 20px;"><a href="user.php?u_id=<?php echo $user_id; ?> " style="text-decoration:none; color: white">回上頁</a></button>
									</td>
								</tr>
								<?php 
									if(isset($_POST['update'])){
										...
										$update = "update user set account='$account',user_name='$u_name',describe_user='$describe_user', password='$u_pass',birthday='$u_birthday',address='$u_address' where user_id='$user_id'";
										$run = mysqli_query($link,$update); 
										if($run){
											echo "<script>alert('您的個人資料已更新！')</script>";
